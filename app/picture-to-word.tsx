...
export default function MatchPicturesScreen() {
  ...
  const handleSuccessfulMatch = (matchedCard: GameCard) => {
    ...
    Animated.parallel([
      ...
      if (settings.playAfterMatch && gameState.matchCard.sound) {
        playWord(gameState.matchCard.sound, { ttsEnabled: settings.textToSpeech, locale, text: gameState.matchCard.text });
      }
  ...
  const performFlipAnimation = () => {
    ...
    Animated.timing(flipAnimation, {
      ...
      // After a short delay, start second flip to show image
  ...
  const renderStaticCard = (card: GameCard, index: number) => {
    ...
    const handleCardTap = () => {
      ...
      // Check if this card matches the current match card (match by image)
  ...
  const renderMatchCard = () => {
    ...
    return (
      ...
      >
        ...
        >
          {/* Front side - Image (initial state) */}
          <Animated.View
            style={[
              styles.cardSide,
              styles.cardBack,
